...
internal sealed class EmployeeService : IEmployeeService
{
    ...
    {
        var company = await repositoryManager.CompanyRepository.GetCompanyAsync(companyId, trackChanges) 
    ...
    { 
        ...
        var employeeDb = await repositoryManager.EmployeeRepository.GetEmployeeAsync(company.Id, employeeId, trackChanges)
    ...
    public async Task DeleteEmployeeForCompanyAsync(Guid companyId, Guid employeeId, bool trackChanges)
    {
        ...
        var employeeForCompany = await repositoryManager.EmployeeRepository.GetEmployeeAsync(company.Id, employeeId, trackChanges) 
            ?? throw new EmployeeNotFoundException(employeeId);
    ...
    {
        var company = await repositoryManager.CompanyRepository.GetCompanyAsync(companyId, compTrackChanges)
            ?? throw new CompanyNotFoundException(companyId);
        
        var employeeEntity = await repositoryManager.EmployeeRepository.GetEmployeeAsync(company.Id, employeeId, empTrackChanges)
            ?? throw new EmployeeNotFoundException(employeeId);
